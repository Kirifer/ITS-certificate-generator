...
const { s3, getPresignedUrl } = require("./config/s3");
      ...
      db.query(
        ...
        (err, results) => {
          if (err || results.length === 0)
            return res
              .status(500)
              .json({ message: "Failed to fetch updated profile" });
          res.json({
...
app.post(
  "/api/pending-certificates",
  upload.single("certificatePng"),
  async (req, res) => {
    try {
      const {
        recipientName,
      ...
      if (!req.file)
        return res.status(400).json({ message: "Certificate PNG is required" });
    ...
    `;
      ...
      db.query(sql, values, (err) => {
        ...
        res
          .status(201)
          .json({ message: "Certificate saved successfully", url: imageUrl });
      });
    } catch (err) {
...
app.get("/api/pending-certificates", async (req, res) => {
  ...
  db.query(sql, async (err, results) => {
    ...
    const filtered = results.filter((cert) => {
      let signatories = cert.approval_signatories || [];
    ...
    const withPresignedUrls = await Promise.all(
      filtered.map(async (cert) => {
        const key = cert.png_path.split(".amazonaws.com/")[1];
        const presignedUrl = await getPresignedUrl(key);
        return { ...cert, png_path: presignedUrl };
      }),
    );

    res.json(withPresignedUrls);
  });
});

// Save pending COC
app.post(
  "/api/pending-cert_coc",
  upload.single("certificatePng"),
  async (req, res) => {
    try {
      const {
      ...
      db.query(
        ...
        (err) => {
          if (err)
            return res
              .status(500)
              .json({ message: "Failed to save certificate" });
          res
            .status(201)
            .json({ message: "Certificate saved successfully", url: imageUrl });
        },
...
// Approve certificate
app.post(
  "/api/approve-certificate-with-signature",
  upload.single("certificatePng"),
  async (req, res) => {
    try {
      ...
      if (!req.file || !certId)
        return res
          .status(400)
          .json({ message: "Missing file or certificate ID" });

      const imageUrl = await uploadToS3(req.file, "approved");

      db.query(
        "SELECT * FROM pending_certificates WHERE id = ?",
        [certId],
        (err, results) => {
          if (err || results.length === 0)
          ...
          const approvalSignatories =
            typeof cert.approval_signatories === "string"
      ...
      `;
          ...
          db.query(
            ...
            (err) => {
              ...
              db.query(
                "DELETE FROM pending_certificates WHERE id = ?",
                [certId],
                (err) => {
                  if (err)
                  ...
                  res.json({
                    message:
                      "Certificate approved and moved to approved_certificates",
                    url: imageUrl,
                  });
                },
              );
  ...
  db.query(selectSql, [certId], (err, results) => {
    ...
    db.query(deleteSql, [certId], (err) => {
      if (err)
        return res
          .status(500)
          .json({ message: "Failed to delete certificate from DB" });
...
app.get("/api/approved-certificates", async (req, res) => {
  const sql = `
    SELECT 
      id,
      recipient_name AS rname,
  ...
  db.query(sql, async (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ message: "Failed to fetch approved certificates" });

    const withPresignedUrls = await Promise.all(
      results.map(async (cert) => {
        const key = cert.png_path.split(".amazonaws.com/")[1];
        const presignedUrl = await getPresignedUrl(key);
        return { ...cert, png_path: presignedUrl };
      }),
...
// Addeed: Proxy certificate image for html2canvas
